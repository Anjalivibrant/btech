...
void withdraw(){
    double withdrawAmt,fee;
    cout<<endl<<"AMOUNT TO BE WITHDRAWN FROM ACCOUNT :"<<endl;
    cin>>withdrawAmt;
     cout<<"There is a small fee charged of 2% when withdrawl amount is greater than 200$."<<endl;
    if(withdrawAmt>amt){
        cout<<"AMOUNT IS OUT OF LIMIT OF THE BALANCE OF Rs."<<amt<<endl;
        cout<<"TRANSACTION FAILED!"<<endl;
    }
    else if(withdrawAmt>200){
        fee=0.02*withdrawAmt;
        with[index1++]=(withdrawAmt+fee);
        fee=0.02*withdrawAmt;
        amt=amt-(withdrawAmt+fee);
    }
    else{
